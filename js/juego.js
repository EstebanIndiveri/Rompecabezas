...
var instrucciones = ["Utilizar las flechas del teclado para mover las piezas.", "Ordenar las piezas hasta alcanzar el objetivo."];
...
function chequearSiGano() {
    ...
    if (grilla.length === grillaGana.length) {
    for (var i = 0; i < grilla.length; i++) {
      for (var j = 0; j < grilla.length; j++) {
    if(grilla[i][j] == grillaGana[i][j]){
      }else{
       return false;
     }
   }
...
function mostrarCartelGanador() {
  ...
  var question = prompt("¿Volver a jugar? \n1: Si \n2: No");
  switch (question) {
    case "1":
    mezclarPiezas(30);
    break;
